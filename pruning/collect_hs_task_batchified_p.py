...
def main(args):
    # Set a default device (used for non-model-parallel operations)
    ...
    def compute_pairwise_cos_similarity(all_logits, use_softmax=True):
        device_local = all_logits.device
    ...
    def compute_pairwise_kl_similarity(all_logits, epsilon=1e-10, use_softmax=True):
        ...
        similarity_matrix = torch.empty((N, N), device=device_local, dtype=torch.float32)
    ...
    # =============LLM Functions============== #
    # ======================================== #
    def calculate_probability(prompt, tokenizer, model, device):
        ...
        labels = inputs["input_ids"][0, 1:length]
        token_log_probs = log_probs[range(length-1), labels]
        sequence_log_prob = token_log_probs.sum().item()
        return sequence_log_prob

    def calculate_probability_batch(prompts, tokenizer, model, device):
        ...
        for i in range(len(prompts)):
            ...
            labels = inputs["input_ids"][i, 1:length]
            token_log_probs = log_probs[range(length-1), labels]
            sequence_log_probs.append(token_log_probs.sum().item())
        return sequence_log_probs

    def get_last_hidden_state(model, tokenizer, prompt, device):
    ...
    def get_best_answer(prompt, answer_choices, tokenizer, model, device):
        prompts = [f"{prompt} {choice}" for choice in answer_choices]
        choice_probs = calculate_probability_batch(prompts, tokenizer, model, device)
    ...
    def get_averaged_logits(prompt, answer, tokenizer, model, device):
    ...
    is_8bit = True
    # Instead of using load_in_8bit with DataParallel,
    # we now use model parallelism via device_map="auto".
    if "11B" not in model_name:
    ...
    # Set max_memory for each GPU (adjust these values based on your hardware)
    max_memory = {}
    num_gpus = torch.cuda.device_count()
    for i in range(num_gpus):
        # For example, if each GPU has around 39GiB available:
        max_memory[i] = "39GiB"

    model = AutoModelForCausalLM.from_pretrained(
        model_name,
        quantization_config=quant_config,
        device_map="auto",   # This enables model parallelism by splitting the model layers across GPUs.
        max_memory=max_memory
    )

    # No need to wrap in DataParallel when using model parallelism.
    ...
    for dataset_path in args.datasets:
        ...
        for i in tqdm(range(len(qa_dataset)), desc=f"Processing QA Dataset {dataset_path}"):
            ...
            best_answer, choice_probs = get_best_answer(question, answer_choices, tokenizer, model, device)
            try:
            ...
            last_hidden_state = get_last_hidden_state(model, tokenizer, question, device)
            logits = get_averaged_logits(question, answer_choices[correct_answer_idx], tokenizer, model, device)
            ...
            all_logits_list.append(logits_tensor.cpu())
        ...
        for matrix, tag in zip(matrices, names):
            ...
            for logits_group in [logits_to_vis, logits_to_vis_f1, logits_to_vis_f2, logits_to_vis_f3, logits_to_vis_f4, logits_to_vis_f5]:
                try:
                    ...
                    log = torch.tensor(np.array([l.cpu().numpy() for l in logits_group]))
